...
class PortraitCharacter {
    constructor(characterData, config, isCpu = false) {
        ...
        this._sheetCache = {}; // Cache sheet status by src
    }
    ...
    _updateRenderRect(baseImg) {
        ...
        // Check cache
        if (this._sheetCache[baseImg.src] === undefined) {
            // Cache logic
            if (baseImg.width > 0) {
                this._sheetCache[baseImg.src] = (baseImg.width >= globalBaseW * 1.5);
            }
        }
        const isSheet = !!this._sheetCache[baseImg.src];
    ...
    _drawImageAutoSlice(ctx, img, rect) {
        ...
        if (isSheet === undefined) {
            ...
            if (this._sheetCache[img.src] === undefined) {
                if (img.width > 0) {
                    const globalBaseW = BattleConfig.PORTRAIT.baseW || 264;
                    this._sheetCache[img.src] = (img.width >= globalBaseW * 1.5);
                }
            }
