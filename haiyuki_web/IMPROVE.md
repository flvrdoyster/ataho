...
- **권장 순서(커버리지 실측 기준으로 재정렬):**
  ...
  4. **`battleRenderer.js` 히트테스트 분리 + `battleScene.js` FX 추출** — 레이아웃 계산 함수(`getVisualMetrics`/`getPlayerHandPosition`/`_actionButtonRect`)를 히트테스트와 **같은 쪽**에 두고 렌더러가 그걸 부르는 방향이어야 좌표가 어긋나지 않는다.
  5. **`assets.js` 렌더 유틸 260줄 → `views/`** — 비트맵 폰트(441-608)와 UI 위젯(609-700). `core/assets`는 매니페스트+로딩+오디오만 남김. 호출부 9곳.
...
- **잔여 1개 — `audio/system_enter`(`skillRegistry.js`, 룰렛 결과 확정):** 대응할 소리가 없고 뭐가 맞는지 근거도 없어 **손대지 않았다**. 원본 확인 필요.
